...
class SpatialMSA(nn.Module):
    def __init__(self, dim_vector, head_dim, heads_number):
        ...
        self.rescale = nn.Parameter(torch.ones(heads_number, 1, 1))
        self.proj = nn.Sequential(
            nn.Linear(head_dim * heads_number, dim_vector),
            nn.GELU()
        )
...
class Cross_Guide_Fusion(nn.Module):
    def __init__(self,in_c_hsi,in_c_msi,in_w,in_h, L):
        super(Cross_Guide_Fusion, self).__init__()

        print("模型：Cross_Guide_Fusion")
        self.min_w=in_w//2
        self.min_c=in_c_hsi
        # stage1   # 31*64*64
        self.spa_sr1=Guided_Spatial_SR(in_c_hsi,in_w,in_h,L,self.min_w*L,in_w//self.min_w)
        self.hsi_up1=nn.Upsample(scale_factor=2, mode='bilinear')
        self.spec_sr1=Guided_Spectral_SR(in_c_hsi,self.min_c,in_c_hsi//self.min_c)

        self.hsi_down_sample1=nn.Conv2d(in_c_hsi,in_c_hsi*2,(4,4),(2,2), 1,bias=False)
        self.msi_down_sample1=nn.Conv2d(in_c_hsi,in_c_hsi*2,(4,4),(2,2), 1,bias=False)

        self.msi_channels_up1=nn.Sequential(
            nn.Conv2d(in_c_msi, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )
        self.hsi_channels_up1= nn.Sequential(
            nn.Conv2d(in_c_hsi, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )


        in_c_hsi, in_w, in_h=in_c_hsi*2, in_w//2, in_h//2    # 62*32*32

        self.spa_sr2 = Guided_Spatial_SR( in_c_hsi, in_w, in_h, L, self.min_w * L, in_w // self.min_w)
        self.hsi_up2 = nn.Upsample(scale_factor=1, mode='bilinear')
        self.spec_sr2=Guided_Spectral_SR(in_c_hsi,self.min_c,in_c_hsi//self.min_c)

        self.hsi_down_sample2=nn.Conv2d(in_c_hsi,in_c_hsi*2,(4,4),(2,2), 1,bias=False)
        self.msi_down_sample2=nn.Conv2d(in_c_hsi,in_c_hsi*2,(4,4),(2,2), 1,bias=False)


        self.msi_channels_up2=nn.Sequential(
            nn.Conv2d(4, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )
        self.hsi_channels_up2=nn.Sequential(
            nn.Conv2d(in_c_hsi//2, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )

        in_c_hsi, in_w, in_h = in_c_hsi * 2, in_w // 2, in_h // 2  #248*8*8

        # cat层
        self.hsi_up3 = nn.Upsample(scale_factor=0.5, mode='bilinear')

        self.hsi_channels_up3= nn.Sequential(
            nn.Conv2d(in_c_hsi//2//2, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )
        self.msi_channels_up3=nn.Sequential(
            nn.Conv2d(4, in_c_hsi, (3, 3), (1, 1), 1, bias=False),
        )
        self.layer_cat=nn.Sequential(
            nn.Conv2d(in_c_hsi * 2, in_c_hsi, (1, 1), (1, 1), padding=0, bias=False),
            nn.LeakyReLU(negative_slope=0.1),
            nn.Conv2d(in_c_hsi, in_c_hsi, (1, 1), (1, 1), padding=0, bias=False),
        )

        self.mask_t1=Self_mask_T(in_c_hsi,in_c_hsi//4,4,2)


        in_c_hsi, in_w, in_h=in_c_hsi//2, in_w*2, in_h*2
        self.connect2 = nn.Conv2d(in_c_hsi*3,in_c_hsi,1,1,0,bias=False)
        self.mask_t2=Self_mask_T(in_c_hsi,self.min_c,in_c_hsi//self.min_c,2)
        self.fu_up2= nn.Sequential(
            nn.Conv2d(in_c_hsi*2, in_c_hsi*2*2, kernel_size=5, stride=1,
                                     padding=5 // 2),
        )


        in_c_hsi, in_w, in_h=in_c_hsi//2, in_w*2, in_h*2
        self.connect3 = nn.Conv2d(in_c_hsi*3,in_c_hsi,1,1,0,bias=False)
        self.mask_t3=Self_mask_T(in_c_hsi,self.min_c,in_c_hsi//self.min_c,2)
        ...
        self.connect4 = nn.Sequential(
            nn.Conv2d(in_c_hsi*3,in_c_hsi,1,1,0,bias=False)
        )
...
if __name__ == '__main__':
    ...
    b=64
    s=2
    hsi = torch.randn((1, 150, a//s, b//s), device=device)
    msi = torch.randn((1, 4, a, b), device=device)

    model2 = Cross_Guide_Fusion(150, 4,a, b, 1).to(device)
    ...
    param=sum(p.numel() for p in model2.parameters() if p.requires_grad)
    print(param)
